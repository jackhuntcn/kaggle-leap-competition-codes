...
for target in weights:
    train[target] = train[target] * weights[target]
...
# force to float32
x = train[FEATCOLS].values.astype(np.float32)
y = train[TARGETCOLS].values.astype(np.float32)

# we will normalize after concat all data

print(x.shape)
print(y.shape)

# save arrays
np.save(f'recreated/user_data/x_{year}_{month}', x)
np.save(f'recreated/user_data/y_{year}_{month}', y)

print(f'{year}-{month} saved x and y')
