...
class LeapModel(nn.Module):
    def __init__(self,
        ...
        self.transformer_layer = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(d_model=input_size, nhead=nhead, dropout=dropout),
        ...
        if hidden_layers and len(hidden_layers):
            first_layer = nn.Linear(lstm_output_size, hidden_layers[0])
            ...
            self.hidden_layers = []
            self.intermediate_layer = nn.Linear(lstm_output_size, self.input_size)
    ...
    def forward(self, x):
        ...
        # LSTM layer
        lstm_output, _ = self.rnn(transformer_output)
        
        # Apply dropout and activation
        x = self.dropout(self.activation_fn(lstm_output))
