...
class LeapModel(nn.Module):
    def __init__(self,
        ...
        lstm1_output_size = hidden_size * 2 if bidirectional else hidden_size

        # Second LSTM layer (after concatenation)
        self.lstm2 = nn.LSTM(input_size=lstm1_output_size + input_size,
                             hidden_size=hidden_size,
        ...
        # Attention layer
        self.attention = AttentionLayer(lstm1_output_size)

        # Hidden layers
        if hidden_layers and len(hidden_layers):
            first_layer = nn.Linear(lstm1_output_size, hidden_layers[0])
            ...
            self.hidden_layers = []
            self.intermediate_layer = nn.Linear(lstm1_output_size, self.input_size)
            self.output_layer = nn.Linear(lstm1_output_size, output_size)
            nn.init.kaiming_normal_(self.output_layer.weight.data)
    ...
    def forward(self, x):
        ...
        # Concatenate original input with LSTM1 output
        combined = torch.cat([x, lstm1_out], dim=-1)
        
        # Second LSTM layer
        lstm2_out, _ = self.lstm2(combined)
        
        # Attention layer
        attn_out = self.attention(lstm2_out)
        
        # Apply dropout and activation
        x = self.dropout(self.activation_fn(attn_out))
