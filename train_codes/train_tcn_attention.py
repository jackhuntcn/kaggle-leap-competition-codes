...
class TCNBlock(nn.Module):
    def __init__(self, input_size, hidden_size, kernel_size, dilation):
        super().__init__()
        self.conv1 = nn.Conv1d(input_size, hidden_size, kernel_size, dilation=dilation, padding='same')
        self.conv2 = nn.Conv1d(hidden_size, hidden_size, kernel_size, dilation=dilation, padding='same')
...
class TCNEncoder(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers):
        ...
        self.blocks = nn.ModuleList([
            TCNBlock(input_size if i == 0 else hidden_size, hidden_size, kernel_size=3, dilation=2**i)
...
class LeapModel(nn.Module):
    def __init__(self,
        ...
        self.encoder = TCNEncoder(input_size, hidden_size, num_layers)
        self.attention = MultiHeadAttention(hidden_size, num_heads, dropout)
        self.layer_norm = nn.LayerNorm(hidden_size)
        
        if hidden_layers and len(hidden_layers):
            first_layer = nn.Linear(hidden_size, hidden_layers[0])
            ...
            self.hidden_layers = []
            self.intermediate_layer = nn.Linear(encoder_output_size, self.input_size)
            self.output_layer = nn.Linear(encoder_output_size, output_size)
            nn.init.kaiming_normal_(self.output_layer.weight.data)
